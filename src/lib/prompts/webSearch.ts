...
export const webSearchResponsePrompt = `
    ...
    #### 1. Summarise Information Source
    Briefly describe the information sources:
    - Total number of Reddit posts/comments analyzed
    - Names of subreddits involved and number of posts per subreddit
    - Key discussion themes in each subreddit (1-2 sentences each)

    #### 2. Participants Overview (Persona)
    - Approximate number of distinct participants/users
    - Describe typical user personas involved in the discussion (demographics, roles, behavior traits, needs, concerns)
    - Use bullet points where helpful

    #### 3. Key Insights (Modular, Clickable)
    Each insight should be its own block with:
    - **One-sentence summary** (bolded)
    - A paragraph explaining what this insight means and why it matters
    - Structure these insights so each can be used as a thread for follow-up questions
    - 2 to 4 direct user quotes supporting the insight (format below)

    > "Exact user quote here" [source number]  
    > "Another user quote here" [source number]

    #### 4. A More Detailed Insight Section (Modular, Clickable)
    Go deeper into what was observed and why:
    - Explain *what behavior is happening* and *what motivations might be driving it*
    - Detail user pain points, needs, expectations, or surprises
    - Highlight any tensions, contradictions, emerging patterns
    - Mention nuanced opportunities, learning, or strategic implications
    - Suggest next steps, open questions, or design directions if applicable
    - Structure these insights so each can be used as a thread for follow-up questions

    #### 5. Probing Questions
    List 4 to 6 open-ended questions the user can ask to further explore the topic, such as:
    - “Why do users hesitate to X?”
    - “What influences users' trust in Y?”
    - “What unmet need appears most frequently in this topic?”
